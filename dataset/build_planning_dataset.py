...
def read_data(config) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    ...
    return conditions, goals, questions, answer


def get_vocabulary(
    conditions: pd.Series, goals: pd.Series, questions: pd.Series, answer: pd.Series
):
...
def get_training_data(
    ...
    data = np.concatenate(
        (conditions.unique(), goals.unique(), questions.unique(), answer.unique())  # type: ignore
    )
...
def add_special_tokens(conditions: pd.Series, goals: pd.Series, questions: pd.Series):
    processed = []
    for condition, goal, question in zip(conditions, goals, questions):
        processed.append(f"[BOC]{condition}[EOC][BOG]{goal}[EOG][BOQ]{question}[EOQ]")
...
def main(config):
    ...
    try:
        conditions, goals, questions, answers = read_data(config)
